...
public class IdCardValidate {
	 ...
	 * 
	 * @param idCard
	 * @param addOneYearAfterBirthday 过了生日就加1岁
	 * @return
	 */
	public final static int calculateAge(String idCard, boolean addOneYearAfterBirthday) {
		int age = 0;

		if (IdCardValidate.isIdcard(idCard)) {
			String year = idCard.length() == 15 ? "19" + idCard.substring(6, 8)
					: idCard.substring(6, 10);
			int iyear = Integer.parseInt(year);
			if (!(iyear < 1900 || iyear > Calendar.getInstance().get(
					Calendar.YEAR))) {
				...
				// 以下逻辑处理 - 过了生日就加1岁
				if (addOneYearAfterBirthday) {
					String month = idCard.length() == 15 ? idCard.substring(8,
							10) : idCard.substring(10, 12);
					String day = idCard.length() == 15 ? idCard.substring(10,
					...
					if (currMonth >= imonth && currDay >= iday) {
						age += 1;
					}
				}
			}
	...
	public final static int getBirthYear(String idCard) {
	...
	public static Integer getGender(String idCard) {
		String genderBit = null;
		if (idCard.length() == 18) {
			genderBit = idCard.substring(16, 17);
		} else if (idCard.length() == 15) {
			genderBit = idCard.substring(14);
		}
		//0:男，1:女
		return Integer.parseInt(genderBit) % 2 == 0 ? 1 : 0;
	}
	 ...
	 * @return
	 */
	public static String getBirdDate(String s) {
		// 年份
